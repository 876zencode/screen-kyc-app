...
const CameraPage: React.FC = () => {
  ...
  const imageType = searchParams.get('type') as 'face' | 'id-front' | 'id-back';
  ...
  const handleCapture = async (type: 'face' | 'id-front' | 'id-back') => {
    const zero = await useZeroClient();

    // Clear previous images of the same type
    const existingImages = await zero.query['capturedImages'];
    for (const row of existingImages) {
      if (row.type === type) {
        await zero.mutate.capturedImages.delete(row.imageUrl);
      }
    }
    ...
    if (photoCanvas) {
      ...
      await zero.mutate.capturedImages.insert({ imageUrl: dataUrl, type });
      console.log(`📸 ${type} image captured and stored in Zero.`);
      setCaptured(true);
    }
  };
              ...
              if (!timeoutRef.current) {
                timeoutRef.current = setTimeout(async () => {
                  if (photoCanvas && video) {
                    ...
                    handleCapture(imageType); // Call handleCapture with the imageType
                  }
                }, 3000);
              }
            } else {
  ...
  return (
    <div className="relative w-full h-screen bg-black">
      ...
      <CameraOverlay type={imageType} highlight={highlight} />
